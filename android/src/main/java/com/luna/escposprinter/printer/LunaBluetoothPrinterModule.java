...
public class LunaBluetoothPrinterModule extends ReactContextBaseJavaModule {
    ...
    public void printImage(String base64, Promise promise) {
        executorService.execute(() -> {
            ...
            try {
                String image = ConverterUtil.convertBase64ToBitmap(printer, base64);
                if (!image.isEmpty()) {
    ...
    public void printCaptainOrder(String printText, Promise promise) {
        ...
        startPrintCaptainOrder(printText, delay, promise);
    }

    private void startPrintCaptainOrder(final String printText, long delay, Promise promise) {
        executorService.execute(() -> {
            EscPosPrinter printer = buildPrinterConnection();
            if (printer == null) {
                return;
            ...
            try {
                int feedValue = (int) mPrinterConfig.getPaperFeed();
                StringBuilder builderText = new StringBuilder();
                builderText.append(printText);
